cargar_tabla_decomisos(null);

function cargar_tabla_decomisos(parametro){
  j$123.ajax({
    ...
    var tabla = "<table id='tabla_8' cellspacing='1' class='tablesorter'>" +
      "<thead>" +
        "<tr>" +
        "<th>Check</th>" +
          "<th>N°</th>" +
      ...
      "</tbody>"+
    "</table>"+
    "<div style='padding-left: 92%'><button id='btn_eliminar_decomisos' class='boton-tab2'>Eliminar</button></div>"+
    "<div id='pager8' class='pager'>"+
      ...
      "</form>"+
    "</div>";
    j$123('#div_tabla_8').html(tabla);
    j$123( "#btn_eliminar_decomisos" ).button();
    if(texthtml != ""){
      $("#tabla_8").tablesorter({widthFixed: true, widgets: ['zebra']}).tablesorterPager({container: $("#pager8")});
    }

    j$123( "#btn_eliminar_decomisos" ).click(function(){
      id = [];

      j$123("#tabla_8 input[type=checkbox]:checked").each(function(){
